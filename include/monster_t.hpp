...
struct monster_t {
  monster_id_t id;
  item_id_t loot_table[MAX_LOOT_TABLE_SIZE]; // Array of item ids if one of them
                                             // is 0 its invalid you can skip
  probability_t probability[MAX_LOOT_TABLE_SIZE]; // Array of proability which
                                                  // maps to the loot_table
...
static std::unordered_map<monster_id_t, std::string> GOBLIN_ID_DATABASE = {
    ...
    {6, "Mirror Shield"}, {7, "Wooden Bow"},  {8, "Yew Bow"},
    {9, "Echo sword"}
