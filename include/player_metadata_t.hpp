...
struct player_metadata_t {
    char player_name[16]; // C string to save this later on disk more easily without serialization
...
uint8_t experience_to_level(const player_metadata_t& player_meta_data) {
    ...
    return static_cast<uint8_t>(std::log(static_cast<double>(xp)) / std::log(1.18));
}

bool save_player_data_on_disk(const std::string& file_name,  player_metadata_t& player_info) {
    auto player_data_handle = fopen(file_name.c_str(), "wb");
    size_t bytes_written = fwrite(reinterpret_cast<uint8_t*>(&player_info), 
                                        sizeof(uint8_t), sizeof(player_metadata_t), player_data_handle);
    return bytes_written == sizeof(player_metadata_t);
}

bool load_player_data_into_memory(const std::string& file_name,  player_metadata_t& player_info) {
    auto player_data_handle = fopen(file_name.c_str(), "rb");
    size_t bytes_written = fread(reinterpret_cast<uint8_t*>(&player_info), 
                                                sizeof(uint8_t), sizeof(player_metadata_t), player_data_handle);
    return bytes_written == sizeof(player_metadata_t);
}
